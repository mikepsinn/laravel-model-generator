...
class SchemaManager implements IteratorAggregate
{
     ...
     */
    public function __construct(ConnectionInterface $connection, $specifiedSchema = null)
    {
        $this->connection = $connection;
        $this->boot($specifiedSchema);
    }

    /**
     * Load all schemas from this connection.
     */
    public function boot($specifiedSchema)
    {
        if (! $this->hasMapping()) {
        ...
        foreach ($schemas as $schema) {
            if($specifiedSchema && $schema !== $specifiedSchema){
                continue;
            }
            $this->make($schema);
        }
