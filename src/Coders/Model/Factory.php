...
class Factory
{
     ...
     * @return $this
     */
    public function on($connection = null, $specifiedSchema = null)
    {
        $this->schemas = new SchemaManager($this->db->connection($connection), $specifiedSchema);
    ...
    public function map($schema)
    {
        ...
        foreach ($mapper->tables() as $blueprint) {
            if($blueprint->isView()){
                print "\n WARNING: Skipping view ".$blueprint->table()."... \n";
                continue;
            }
    ...
    private function formatBaseClasses(Model $model)
    {
        ...
        return "use {$model->getBaseNamespace()}\\{$this->getBaseClassName($model)};";
    }
